...
- **可交互回放**：动作日志升级为整局时间线，支持逐步播放 / 暂停 / 上一步 / 下一步 / 拖动任意跳转；
  逐帧重建路线、战斗（手牌/血量/护盾/状态）、锻造与商店交易状态；每帧携带规则版本与校验点
  （状态哈希），自动兼容旧日志（裸 id 手牌按出现序映射），回放全程只读——不写存档、不触发失败解锁
...
交易贯通后续战斗与回放；**可交互回放**：逐帧视口与实时视口一致、规则版本/校验点哈希、篡改告警不崩溃、
旧日志（裸 id）兼容、回放不写 runs/events/profile、战败回放不重复解锁、锻造与买卖移除状态重建。
...
- `GET  /api/runs/{id}/replay` 可交互回放（整局时间线：逐帧视口 + 结算事件 + 校验点；仍兼容旧字段 `actions`）
...
- 可交互回放（`app/replay.py` + `frontend/src/replay/`）：
  - 重建器从“建局空状态”开始，按 `battle_events` 调用与实时对局共用的纯推进函数
    `service._apply_action`（不含任何持久化），逐帧生成完整 `_public_view`，
    因此路线/战斗/锻造/交易任意一帧都能直接渲染，支持播放/暂停/跳转；
  - 同一场战斗的连续帧带相同 `battle_key`，前端据此保持 Phaser 场景挂载，
    顺序前进播放结算动画，跳转则直接落该帧权威快照（`instant`）；
  - 每个动作落库时带 `rule_version`（当前 `2024.2-card-instances`，见 `app/rules.py`）
    与 `checkpoint`（状态 JSON 的 sha256 前 16 位），回放逐帧比对；旧日志无这些字段时
    标记 `legacy`（规则版本回退到 `2024.1-card-ids`）、跳过校验、裸 id 手牌按手牌出现序
    稳定映射到实例 uid；单步无法重放时降级为“分叉帧”并告警，绝不阻断整条时间线；
    终帧与数据库最终状态再做一次哈希对账，地图可用种子重新生成旁证；
  - 隔离：回放只操作内存深拷贝，不写 `runs`/`battle_events`/`profile`，战败不触发失败解锁，
    视口以 `include_profile=False` 构建（连解锁表都不读取）。
